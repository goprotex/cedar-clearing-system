...
export default function MapContainer({ accessToken }: MapContainerProps) {
  ...
  useEffect(() => {
    ...
    // Toggle cedar AI overlay (fill + border) — show whenever toggled (can stack with hologram)
    ...
    // Keep classification grid above pasture tint / NAIP
    if (cedarVisible && map.getLayer('cedar-fill')) {
      map.moveLayer('cedar-fill');
      if (map.getLayer('cedar-border')) map.moveLayer('cedar-border');
    }
    ...
    // Soil must be above NAIP/satellite (runs after 3d-trees exists in hologram mode)
    if (layers.soil && map.getLayer('soil-overlay')) {
      if (layers.hologram && map.getLayer('3d-trees')) {
        map.moveLayer('soil-overlay', '3d-trees');
      } else {
  ...
  const toggleLayer = useCallback((key: LayerKey) => {
    setLayers((prev) => {
      ...
      // Hologram: enable terrain, switch to NDVI base map (AI Cedar overlay stays user-controlled)
