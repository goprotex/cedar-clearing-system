...
/** Global scale for hologram tree meshes (canopy, trunk, shadows, dots) */
const TREE_VIS_SCALE = 0.75;
  ...
  private rebuildMeshes() {
    ...
    for (const sp of ['cedar', 'oak', 'mixed'] as Species[]) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        const radiusX = (t.canopyDiameter / 2) * TREE_VIS_SCALE;
        const radiusZ = (t.canopyDiameter / 2) * TREE_VIS_SCALE;
        // Oak: raise canopy above trunk
        const baseY = sp === 'oak' ? t.height * 0.4 * TREE_VIS_SCALE : 0;
        dummy.position.set(pos.x, baseY, pos.z);
        dummy.scale.set(radiusX, t.height * (sp === 'oak' ? 0.6 : 1) * TREE_VIS_SCALE, radiusZ);
        dummy.updateMatrix();
      ...
      for (let i = 0; i < arr.length; i++) {
        const t = arr[i];
        const pos = this.lngLatToScene(t.lng, t.lat);
        const trunkH = (sp === 'oak' ? t.height * 0.5 : t.height * 0.3) * TREE_VIS_SCALE;
        dummy.position.set(pos.x, 0, pos.z);
        dummy.scale.set(t.canopyDiameter * 0.15 * TREE_VIS_SCALE, trunkH, t.canopyDiameter * 0.15 * TREE_VIS_SCALE);
        dummy.updateMatrix();
      ...
      for (let i = 0; i < arr.length; i++) {
        const t = arr[i];
        const pos = this.lngLatToScene(t.lng, t.lat);
        const r = t.canopyDiameter * 0.6 * TREE_VIS_SCALE;
        dummy.position.set(pos.x, 0, pos.z);
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        const pos = this.lngLatToScene(t.lng, t.lat);
        dummy.position.set(pos.x, 0, pos.z);
        dummy.scale.set(3 * TREE_VIS_SCALE, 1, 3 * TREE_VIS_SCALE);
        dummy.updateMatrix();
  ...
  private rebuildMarkerRings() {
    ...
    const buildRings = (trees: MarkedTree[], material: THREE.ShaderMaterial): THREE.InstancedMesh => {
      ...
      for (let i = 0; i < trees.length; i++) {
        const t = trees[i];
        const p = this.lngLatToScene(t.lng, t.lat);
        const ringSize = Math.max(t.canopyDiameter * 1.5 * TREE_VIS_SCALE, 8 * TREE_VIS_SCALE);
