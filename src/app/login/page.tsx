...
function authErrorMessage(e: unknown): string {
  if (e instanceof TypeError && /failed to fetch/i.test(String(e.message))) {
    return (
      'Cannot reach Supabase (network or DNS). Confirm NEXT_PUBLIC_SUPABASE_URL in your deployment matches ' +
      'an active project: Supabase Dashboard → Project Settings → API → Project URL, then redeploy. ' +
      'Paused, deleted, or mistyped projects do not resolve.'
    );
  }
  if (e instanceof Error) {
    ...
    if (/failed to fetch|networkerror|load failed/i.test(m)) {
      return (
        'Cannot reach Supabase. Check NEXT_PUBLIC_SUPABASE_URL on the host (e.g. Vercel) and that the Supabase project is running.'
      );
    }
    return m;
  }
