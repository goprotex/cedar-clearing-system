...
export default function BidClientLinker() {
  ...
  if (loading) {
    return <p className="text-[10px] font-mono text-[#5a4136] mb-3">LOADING_CRM…</p>;
  }
  ...
  const cid = currentBid.clientId ?? '';
  const missingFromList = Boolean(cid && !clients.some((c) => c.id === cid));
  ...
  return (
    <div className="mb-3 space-y-2">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
        <select
          aria-label="Link bid to CRM client"
          value={cid}
          onChange={(e) => void handleSelectChange(e.target.value)}
          className="h-8 min-w-0 sm:min-w-[220px] rounded-md border border-[#353534] bg-[#201f1f] px-2 text-sm text-[#e5e2e1] focus:border-[#FF6B00] focus:outline-none"
        >
          <option value="">— No CRM link —</option>
          {missingFromList ? (
            <option value={cid}>Linked client (reload list if missing)</option>
          ) : null}
          {clients.map((c) => (
        ...
        {cid ? (
          <Link href={`/clients/${cid}`} className="text-[10px] font-mono text-[#FF6B00] hover:underline shrink-0">
            Open profile →
          </Link>
        ) : null}
      </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="border-[#353534] bg-[#1a1a1a] text-[#e5e2e1] max-w-md">
          ...
          <p className="text-[10px] text-[#a98a7d]">Contact fields on this bid are copied to the new client record.</p>
